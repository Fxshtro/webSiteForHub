...
export default function EntrancePage(): React.JSX.Element {
  ...
  const handleSubmit = useCallback(async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const username = (form.elements.namedItem("login") as HTMLInputElement).value.trim();
            ...
            <div className="mt-[40px] flex h-full sm:w-[600px] flex-col justify-between px-5 pt-15 pb-8 sm:h-auto sm:px-0 sm:block">
              <form onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  {error && (
                    <p className="mt-4 text-center text-red-400 text-[18px]" role="alert">
                      {error}
                    </p>
                  )}
                  ...
                  <button
                    ...
                    hover:[box-shadow:0px_0px_150px_#ffffff56,_inset_0px_0px_35px_#ffffff95] active:[box-shadow:0px_0px_150px_#0000006f,_inset_0px_0px_35px_#00000062] sm:py-1 sm:text-[46px] sm:rounded-2xl sm:rounded-3xl disabled:opacity-50"
                  >
                    {loading ? "Вход..." : "Войти"}
                  </button>
                </div>
